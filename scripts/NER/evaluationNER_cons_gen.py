...
if args.dataset == "uner" and args.uner_subset == "all":
    print(
        "\n" + "=" * 78 + "\n"
        "TODO -- the semin_*_f1 numbers in the CSV above are NOT yet the ones that\n"
        "belong next to Semin et al.'s table. They pool all 18 treebanks into one\n"
        "micro F1; their headline figure is the MACRO average of the 18 per-treebank\n"
        "F1s.\n\n"
        "Compute it from the predictions once all runs are finished: every JSONL line\n"
        "carries `dataset_names` plus its own semin_hard/soft_overlap|predicted|gold\n"
        "counts, so group by treebank, sum the counts, F1 per treebank, then average\n"
        "the 18. No re-running needed.\n"
