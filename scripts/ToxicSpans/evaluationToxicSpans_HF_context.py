"""
Context-based (prompting) baseline on Toxic Spans.

Mirror image of `evaluationToxicSpans_cons_gen.py` -- same model registry, reasoning handling, seeds, sampling,
sharding, dataset objects and metrics. The model is shown the ORIGINAL post verbatim (newlines and repeated spaces
included), exactly as in the constrained arm, so gold character offsets are used as they come and the two arms are
scored on identical inputs. Only the requested output format differs: a JSON array of {entity, label, context} objects
instead of inline markup over a verbatim copy.

One arm, one prompt: exact matching, `SYSTEM_PROMPT_CONTEXT_BASE_TOXIC_SPANS` (the constrained prompt with the task and
label definition kept word for word and only the output format swapped). The thesis-era fuzzy/prompt sweep is not
repeated.

Metrics, matching the constrained script:
- character-level F1 averaged over posts (the thesis metric): P = |pred ∩ gold| / |pred|, R = |pred ∩ gold| / |gold|.
  Empty gold + empty pred -> F1=1.0; empty gold + non-empty pred -> F1=0.0.
- Semin et al.'s pooled (micro) character-overlap F1, hard and soft. Toxic Spans has one label, so the two coincide.
- token budget and throughput, and the reasoning termination counters.

Plus the failure modes that exist only on this side of the comparison and are the reason the arm is run: spans the
model named that are not in the post (`span_not_found`), labels outside the tagset (`invalid_label`), and responses
that did not parse (`format_invalid`) -- the first two structurally impossible under constrained generation, the third
a weaker test than its wrong-text rate.
"""
import argparse
import os
import random
...
from utils.span_datasets import load_toxic_spans, TOXIC_LABELS
...
from utils.system_prompts import SYSTEM_PROMPT_CONTEXT_BASE_TOXIC_SPANS
from utils.model_registry import get, resolve_sampling

# -------------------------
# Model configuration
# -------------------------
parser = argparse.ArgumentParser("Evaluate the context-based prompting baseline on Toxic Spans.")
...
valid_labels = set(TOXIC_LABELS)

# -------------------------
# Load dataset
# -------------------------
print("Loading heegyu/toxic-spans test split...")
raw = load_toxic_spans()
...
pred_fh = open_jsonl_writer(
    f"{RESULTS_ROOT}/ToxicSpans/Context-Based/Predictions/"
    f"toxic{shard_suffix()}_{model_short}_think_{args.enable_thinking}_{sampling_strategy}"
...
for seed in SEEDS:
    ...
    for idx in tqdm(range(len(sampled)), desc=f"seed {seed}", file=sys.stdout):
        example = sampled[idx]
        # The ORIGINAL post, shown verbatim -- no whitespace rebuild, same string the constrained arm copies.
        ...
        try:
            generated, num_output_tokens, generation_seconds = generate_markup(
                ...
                system_prompt=SYSTEM_PROMPT_CONTEXT_BASE_TOXIC_SPANS,
        ...
        # Collect the failure counts for this post, to be averaged over the seed at the end.
        ...
        # Character-level F1 for this post, to be averaged over the seed at the end.
        ...
        # Semin et al.'s micro counts, hard and soft. For Toxic Spans the two coincide (one label), but both are
        ...
        if (idx + 1) % EVAL_INTERVAL == 0:
            ...
            tqdm.write(
                ...
                f"charF1={statistics.mean(char_f1_per_post):.4f} hardF1={partial_hard['f1']:.4f} | "
...
results_path = (f"{RESULTS_ROOT}/ToxicSpans/Context-Based/Csv/"
                f"toxic{shard_suffix()}_{model_name.split('/')[-1]}_{BATCH_SIZE}_BS_{config_tag()}_{sampling_strategy}.csv")
