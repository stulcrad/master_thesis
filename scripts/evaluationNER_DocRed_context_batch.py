...
import time
from utils.context_matching_utils import json_safe_parse, assign_entities_from_context, docred_to_bio_tags
...
MAX_EXAMPLES = 64 # Full evaluation
N_ITERS = 3
EVAL_INTERVAL = 8 # Log after every 10 iterations
BATCH_SIZES = [1, 2, 4]
...
prompts = {
    ...
    SYSTEM_PROMPT_DOCRED_MD: "SYSTEM_PROMPT_DOCRED_MD",
}
            ...
            for model_name in ["gemma3:4b", "qwen3:8b", "gpt-oss:20b", "llama3.1:8b"]:
                ...
                for exp_id in range(N_ITERS):
                    ...
                    for batch_idx in tqdm(range(num_batches), file=sys.stdout):
                        ...
                        pred_tags = assign_entities_from_context(batch_tokens, pred_json, fuzzy=FUZZY, fuzzy_threshold=FUZZY_THRESHOLD, matching_type='anchor')
